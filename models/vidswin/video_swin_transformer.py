...
class SwinTransformer3D(nn.Module):
    ...
    def inflate_weights(self, logger):
        ...
        state_dict['patch_embed.proj.weight'] = \
            state_dict['patch_embed.proj.weight'].unsqueeze(2).repeat(1, 1, self.patch_size[0], 1, 1) / self.patch_size[
                0]
    ...
    def forward(self, x):
        ...
        return x  # [B, C, T, H, W], T does not downsample so we can get the per-frame feature
...
class VideoSwinTransformerBackbone(nn.Module):
    ...
    def forward(self, samples: torch.Tensor, num_frames):
        ...
        out = {}  # layer outputs before downsampling
...
def vidswin_model(model_name: AnyStr, model_path: AnyStr = None):
    assert model_name in configs.keys(), f"Model name '{model_name}' not in {configs.keys()}"
